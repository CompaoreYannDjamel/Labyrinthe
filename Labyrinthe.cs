...
using static TP1_INF1008.Model.Node;
...
namespace TP1_INF1008
{
    public partial class Labyrinthe : Form
    {
        ...
        private HashSet<Liaison> finalLiaisons = null;
        ...
        public Node GetNoeud(int posX, int posY)
        {
            map.isValideXY(posX, posY);
            return new Node(this, posX, posY);
        }
        ...
        public HashSet<Liaison> Prim()
        {
            ...
            finalLiaisons = new HashSet<Liaison>();
            HashSet<Node> finalNode = new HashSet<Node>();

            Node nodeToAdd = GetNoeud(0, 0);
        ...
        public string AffichageLabyrinthe()
        {
            ...
            Liaison[][] screen = new Liaison[GetMap().GetWidh * 2 - 1][];
            for (int y = 0; y < (GetMap().GetWidh * 2) - 1; y++)
            {
                screen[y] = new Liaison[(GetMap().GetLength * 2) - 1];
            }

        
            Liaison tmp;
            foreach (Liaison item in finalLiaisons)
            {
                tmp = item;
